...
module Stats
  describe R301OffenderSarOpenCasesReport do
    after(:all) { DbHousekeeping.clean(seed: true) }

    describe '.title' do
      it 'returns correct title' do
        expect(described_class.title).to eq 'Open cases report for Offender SAR'
      end
    end
    ...
    describe '.description' do
      it 'returns correct description' do
        expect(described_class.description)
          .to eq 'The list of open Offender SAR cases within allowed and filtered scope'
      end
    end

    describe '#case_scope' do
      before do
        create_report_type(abbr: :r301)
      end
      ...
      it 'returns only Offender SAR cases with initial scope of nil' do
        ...
        expect(report.case_scope).to match_array( [@offender_sar_1, @offender_sar_3, @offender_sar_4])
      end
