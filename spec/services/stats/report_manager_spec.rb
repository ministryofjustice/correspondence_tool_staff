  ...
  describe ReportManager do
    ...
    describe '#report_class' do
      it 'returns the class of the report with the given index' do
        expect(manager.report_class('R003')).to eq R003CurrentMonthCaseOverviewReport
      end
    end

    describe '#report_object' do
      it 'returns an instantiated oject' do
        expect(manager.report_object('R003')).to be_instance_of(R003CurrentMonthCaseOverviewReport)
      end
    end

    describe '#filename' do
      it 'returns the filename for the given report' do
        expect(manager.filename('R003')).to eq 'r003_current_month_case_overview_report.csv'
      end
    end
  end
end
