...
describe CaseAutoCloseService do
  ...
  describe ".call" do
    context "when not dry-run (dryrun: false)" do
      ...
      it "auto-closes cases stopped for more than 90 days only" do
    ...
    context "when dry-run (dryrun: true)" do
      it "does not change the state of eligible cases" do
        expect { described_class.call(dryrun: true) }.not_to(change { kase_well_over_threshold.reload.current_state })
      end
    end
  end
end
