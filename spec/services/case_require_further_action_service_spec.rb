  ...
  describe '#call' do
    ...
    context 'require further action for ico (FOI) with valid parameters' do
      ...
      it 'Same responder' do
        params = {
          ...
          "external_deadline": Date.today + 10.day
        }
        service = CaseRequireFurtherActionService.new(user, kase, params)
        ...
        allow(state_machine).to receive(:teams_that_can_trigger_event_on_case!).with(
          ...
          user: user).and_return([managing_team])
    ...
    context 'require further action for ico (FOI) with invalid params' do
      it 'raises an error when it saves' do
        params = {
          "message": 'require further action',
          "internal_deadline": Date.today + 10.day,
