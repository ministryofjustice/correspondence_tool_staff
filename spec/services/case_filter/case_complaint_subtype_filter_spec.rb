...
describe CaseFilter::CaseComplaintSubtypeFilter do
  ...
  describe '#applied?' do
    ...
    context 'filter_case_complaint_subtype present' do
      let(:search_query)      { create :search_query,
                                       filter_complaint_subtype: ['standard_complaint'] }
    ...
    Case::SAR::OffenderComplaint.complaint_subtypes.each do | complaint_subtype, _ |
      describe 'filtering for standard complaint cases' do
        ...
        it 'returns the correct list of cases' do
          complaint_case = create :offender_sar_complaint, complaint_subtype: complaint_subtype
  ...
  describe '#crumbs' do
    ...
    context 'filtering for cases' do
      ...
      context 'filtering for one complaint subtype' do
        let(:search_query)      { create :search_query,
                                         filter_complaint_subtype: ['missing_data'] }
        ...
        describe 'params that will be submitted when clicking on the crumb' do
          subject { case_complaint_subtype_filter.crumbs[0].second }

          it { should eq 'filter_complaint_subtype' => [''],
      ...
      context 'filtering for more than one type' do
        let(:search_query)      { create :search_query,
                                         filter_complaint_subtype: ['inaccurate_data', 'redacted_data'] }
  ...
  describe '.process_params!' do
    it 'processes filter_complaint_subtype, sorting and removing blanks' do
      ...
      expect(params).to eq filter_complaint_subtype: [
                             'inaccurate_data',
                             'missing_data',
                             'timeliness',
                           ]
