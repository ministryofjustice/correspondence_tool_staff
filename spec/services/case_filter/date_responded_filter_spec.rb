...
describe CaseFilter::DateRespondedFilter do
  ...
  before(:all) do
    Timecop.freeze(Time.zone.local(2018, 4, 26, 14, 57, 0)) do
      @kase_1 = create :case, date_responded: 30.working.days.ago
      @kase_2 = create :case, date_responded: 25.working.days.ago
      @kase_3 = create :case, date_responded: 100.working.days.ago
    end
  end
  ...
  describe "#applied?" do
    ...
    context "when no date_responded present" do
      let(:search_query)      { create :search_query }

      it { is_expected.not_to be_applied }
    end

    context "when both from and to date_responded present" do
      let(:search_query)      do
        create :search_query,
    ...
    context "when only date_responded_from present" do
      let(:search_query) do
        create :search_query,
               date_responded_from: Time.zone.today
      end
    ...
    context "when only date_responded_to present" do
      let(:search_query) do
        create :search_query,
               date_responded_to: Time.zone.today
      end
  ...
  describe "#call" do
    ...
    context "when no cases with date_responded in date range" do
      let(:search_query) do
        create :search_query,
    ...
    context "when case with date_responded within date rang" do
      let(:search_query) do
        create :search_query,
  ...
  describe "#crumbs" do
    ...
    context "when no date_responded from or to selected" do
      let(:search_query) do
        create :search_query,
    ...
    context "when from and to date selected" do
      ...
      describe "params that will be submitted when clicking on the crumb" do
        it {
          expect(filter.crumbs[0].second).to eq "date_responded_from" => "",
                                                "date_responded_to" => "",
