...
feature "Upload commissioning document", skip: true do
  ...
  given!(:commissioning_document) { create(:commissioning_document, data_request:) }
  ...
  scenario "upload a different filetype", js: true do
    ...
    upload_commissioning_document_page.upload_document_button.click
    expect(data_request_show_page).to be_displayed
    expect(data_request_show_page).to have_content(I18n.t("notices.no_commissioning_document_uploaded"))
    last_file = offender_sar_case.attachments.last
    expect(last_file).to be_nil
  end
end
