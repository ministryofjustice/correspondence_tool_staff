...
feature 'when extending a SAR case deadline' do
  ...
  context 'a manager' do
    ...
    scenario 'extending a SAR case by 30 days twice then removing extension deadline' do
      ...
      expected_initial_extension_date = get_expected_deadline(2.month.since(received_date)).strftime('%-d %b %Y')
      expected_final_extension_date = get_expected_deadline(3.month.since(received_date)).strftime('%-d %b %Y')
    ...
    scenario 'extending a SAR case by 60 days' do
      ...
      # 1. Extend by 60 days
      extend_sar_deadline_for(kase: kase, num_calendar_months: 2) do |page|
