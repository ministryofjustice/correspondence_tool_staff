...
RSpec.describe RetentionSchedule, type: :model do
  ...
  describe "state transitions" do
    ...
    it "disallows incorrect transitions" do
      expect(retention_schedule).not_to transition_from(:not_set, :retain, :review)
        .to(:destroyed).on_event(:anonymise)

      expect(retention_schedule).not_to transition_from(:review, :to_be_anonymised)
      ...
      expect(retention_schedule).not_to allow_transition_to(
  ...
  describe "validations" do
    it { is_expected.to belong_to(:case).class_name("Case::SAR::Offender") }
    it { is_expected.to validate_presence_of(:case) }
    it { is_expected.to validate_presence_of(:planned_destruction_date) }
  ...
  describe "class methods" do
    describe "date ranges" do
      ...
      it 'returns a range that is correct to view triagable "destroy" cases' do
        ...
        expect(class_range).to be(Range)
    ...
    describe ".state_names" do
      it "returns an array of symbols of all possible states" do
        expect(
          ...
          %i[not_set retain review to_be_anonymised anonymised],
        )
      end
    end
  end
