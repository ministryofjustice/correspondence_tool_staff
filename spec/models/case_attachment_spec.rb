...
RSpec.describe CaseAttachment, type: :model do
  let!(:time) { Time.zone.local(2017, 6, 14, 14, 22, 3) }

  describe "type enum" do
    it { is_expected.to have_enum(:type).with_values %w[response request ico_decision commissioning_document] }
    it { is_expected.to validate_presence_of :type }
  end

  describe "key" do
    describe "file extension validation" do
      it { is_expected.to allow_file_extension("pdf")  }
      it { is_expected.to allow_file_extension("doc")  }
      it { is_expected.to allow_file_extension("docx") }
      it { is_expected.to allow_file_extension("xls")  }
      it { is_expected.to allow_file_extension("xlsx") }

      it { is_expected.not_to allow_file_extension("exe") }
      it { is_expected.not_to allow_file_extension("com") }
      it { is_expected.not_to allow_file_extension("bat") }
    end
