...
RSpec.describe Assignment, type: :model do
  ...
  describe "scope unapproved" do
    it "returns assignments that have not been approved" do
      ...
      responded_trigger_case
      approved_trigger_case
      expect(described_class.unapproved)
  ...
  describe "scope last_responding" do
    ...
    let(:kase) do
      create(:case).tap do |kase|
        kase.assignments << assignment1
        kase.assignments << assignment2
      end
    end
  ...
  describe "#reasons_for_rejection" do
    it "is required for rejection" do
      expect(described_class.new(state: "rejected"))
        .to validate_presence_of(:reasons_for_rejection)
    end

    it "is not required for acceptance" do
      expect(described_class.new(state: "accepted"))
  ...
  context "when approved validation" do
    ...
    context "and non-approving_role" do
      ...
      it "is invalid when true" do
        ...
        expect(assignment.errors[:approved]).to eq %w[true]
      end
    end
  end
