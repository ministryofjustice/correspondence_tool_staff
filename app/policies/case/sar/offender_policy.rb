class Case::SAR::OffenderPolicy < Case::SAR::StandardPolicy
  ...
  def extend_sar_deadline?
    clear_failed_checks
    user.team_admin? && clear_failed_checks && check_user_can_manage_offender_sar && check_can_trigger_event(:extend_sar_deadline)
  end

  def remove_sar_deadline_extension?
    clear_failed_checks
    user.team_admin? && clear_failed_checks && check_user_can_manage_offender_sar && check_can_trigger_event(:remove_sar_deadline_extension)
  end
