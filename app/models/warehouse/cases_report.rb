  ...
  class CasesReport < ApplicationRecord
    ...
    MAPPINGS = {
      ...
      'CaseClosure': {
        ...
        execute: ->(record, query){ CasesReport.find_cases(record, query, :metadata_id) },
      },
      'CaseTransition': {
        fields: [],
        parameter: nil,
      ...
      'Team': {
        ...
        parameter: :team_id,
        execute: ->(record, query){ CasesReport.find_cases(record, query, :team_id) },
      },
      'TeamProperty': {
        fields: %w[
        ...
        execute: ->(record, query){ CasesReport.find_cases(record, query, :property_id) },
      },
      'User': {
        fields: %w[
            creator_id
        ...
        parameter: :user_id,
        execute: ->(record, query){ CasesReport.find_cases(record, query, :user_id) },
      },
    }.freeze
    ...
    class << self
      ...
      def execute_setting(record, setting)
        ...
        setting[:execute]
          .call(record, query)
          .each { |kase| self.generate(kase) }
      end

      def process_cases(query)
        count = 0
      ...
      def find_cases(record, query, parameter)
        self
          .includes(:case)
